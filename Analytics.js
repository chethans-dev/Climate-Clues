/**
 * Climate Clues Analytics Engine
 * Tracks user interactions, search trends, and system performance.
 */
class ClimateAnalytics {
  ...
  trackError(message) {
    ...
    // In a real app, this would send to a telemetry endpoint
  }

  saveAnalytics() {
    // Keep only last 100 searches for local storage
  ...
  getStats() {
    ...
    const failed = this.searchHistory.filter(
      (s) => s.status === "error",
    ).length;
    ...
    return {
      ...
      recentCities: [...new Set(this.searchHistory.map((s) => s.city))].slice(
        -5,
      ),
    };
  }
